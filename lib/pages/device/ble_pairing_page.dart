...
class _BlePairingPageState extends State<BlePairingPage> {
  ...
  Future<void> _startPairing(
    ...
    if (char == null || !mounted) {
      setState(() {
        _step = BlePairingStep.failed;
        _errorMessage = l.deviceNotSupport;
      });
      return;
    }
    ...
    if (isBleDirectConnect) {
      ...
      try {
        ...
        debugPrint('[配网][下发前] thing.network.set 原始内容: $contentJson');

        // 不加密，直接按 JSON 发送（对齐 ai_plush_companion 风格）
        sent = await _bleService.sendPairingData(char, content);
    ...
    if (!sent || !mounted) {
      setState(() {
        _step = BlePairingStep.failed;
        _errorMessage = l.sendPairingFailed;
      });
      return;
    }
    if (!isBleDirectConnect) {
          ...
          child: Column(
            ...
            children: [
              ...
              if (_scannedDevices.isNotEmpty) ...[
                ...
                Text(
                  l.foundDevices(_scannedDevices.length),
